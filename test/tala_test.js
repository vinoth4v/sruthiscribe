...
console.log('--- no-crash-on-load (this is what was actually broken) ---');
  ...
  someLaterNote.click();
  await new Promise(z=>setTimeout(z,50));
  const setBeat1 = [...r.d.querySelectorAll('.picker button')].find(b=>/avartana here|Set as beat 1/.test(b.textContent));
  chk('the anchor control exists in the picker for a non-anchor note', !!setBeat1);
  setBeat1.click();
  await new Promise(z=>setTimeout(z,50));
